...
class BaseHeuristic:
    def __init__(self, graph: UndirectedGraph,
                 nodeFromID: Node, nodeToID: Node) -> None:
        self.graph = graph
...
class EuclideanForTube(BaseHeuristic):
    def __init__(self, graph: UndirectedGraph,
                 nodeFrom: Node, nodeTo: Node) -> None:
...
class StationCount(BaseHeuristic):
    ...
    def h(self, currentNode: Node) -> float:
        # return self.state.get(currentNode.id, self.state[self.nodeFrom.id])
        return self.distTo.get(currentNode.id)
    ...
    def _bfs(self):
        ...
        # goal station is 0 stations away from itself
        self.distTo[self.nodeTo.id] = 0
        ...
        while (firstOut != len(queue)):  # while it's not empty
            ...
            # if n.id == self.nodeFrom.id: #should full bfs be run until
            # origin station is found to label every node in graph or
            # should nodes farther than origin be defaulted to inf?
